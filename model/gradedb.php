<?php
     ...
     * @version 1.0
     *
CREATE TABLE grades ( grade_id int NOT NULL AUTO_INCREMENT PRIMARY KEY,
        student_id int NOT NULL, exercise_id int NOT NULL, grade int ); */

    //SELECT students.fName, students.lName, exercises.exercise_name,
    //grades.grade FROM grades INNER JOIN students ON grades.student_id =
    //students.student_id INNER JOIN exercises ON grades.exercise_id =
    //exercises.exercise_id ORDER BY grades.student_id
    ...
    class GradesDB {
        ...
        function addGrade($exercise_id, $grade) {
            $insert = 'INSERT INTO grades (exercise_id, grade) VALUES
            (:exercise_id, :grade)';
        ...
        function getGradeID($student_id, $exercise_id) {
            $insert = 'SELECT grade_id FROM `grades` WHERE student_id =
            :student_id AND exercise_id = :exercise_id';
        ...
        function updateGrade($student_id, $exercise_id, $grade) {
            $update =
                'INSERT INTO grades (grade_id, student_id, exercise_id, grade)
                VALUES (:grade_id, :student_id, :exercise_id, :grade) ON
                DUPLICATE KEY UPDATE grade = :grade';
            ...
            if(!empty($grade_id)) {
                ...
                $grade_id = 1; $statement->bindValue(':grade_id', $grade_id,
                PDO::PARAM_INT);
            }
        ...
        ////READ
        //function allGrades() {
        //    $select = 'SELECT * FROM grades ORDER BY student_id'; $results =
        //    $this->_pdo->query($select);
        //
        //    $resultsArray = array();
        //
        //    //map each student id to a row of data for that student while
        ...
        //READ
        function allGrades() {
            $select = 'SELECT grades.grade_id, grades.student_id,
            students.fName, students.lName, exercises.exercise_name,
            grades.grade FROM grades INNER JOIN students ON grades.student_id =
            students.student_id INNER JOIN exercises ON grades.exercise_id =
            exercises.exercise_id ORDER BY grades.student_id'; $results =
            $this->_pdo->query($select);
